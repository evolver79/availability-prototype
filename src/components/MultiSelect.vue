<script setup>
/**
 * MultiSelect — uses the native Popover API for the dropdown.
 * The popover lives in the top layer, so it naturally escapes
 * any overflow:hidden or modal containers.
 */
...
const filteredOptions = computed(() => {
  ...
  return q
    ? props.options.filter((o) => o.label.toLowerCase().includes(q) || (o.sublabel && o.sublabel.toLowerCase().includes(q)))
    : props.options
})
...
const groupedOptions = computed(() => {
  ...
  for (const type of typeOrder) {
    ...
    if (items.length > 0) groups.push({ label: typeLabels[type] || type, items })
  }
...
onMounted(() => {
  // Close when clicking outside (popover auto handles this for auto, but we use manual)
  ...
  <div class="relative">
    ...
    >
      ...
      <span
        ...
        :class="chipColor(opt)"
      >
        <span v-if="warningIds.has(opt.id)" class="w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0"></span>
        <span class="truncate">{{ opt.label }}</span>
        <button @click.stop="remove(opt.id)" class="ml-0.5 text-current opacity-50 hover:opacity-100 transition-opacity">
    ...
    <!-- Dropdown: native popover, lives in top layer -->
    <div
      ref="popoverRef"
      popover="manual"
      @toggle="onPopoverToggle"
      class="m-0 p-0 bg-white border border-gray-200 rounded-lg shadow-lg max-h-56 overflow-y-auto"
    >
      ...
      <template v-for="group in groupedOptions" :key="group.label">
        ...
        >
          <div
            ...
            :class="modelValue.includes(option.id) ? 'bg-blue border-blue' : 'border-gray-300'"
          >
            <svg v-if="modelValue.includes(option.id)" class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
